...
def screenshot_page(path, settle_ms=0, wait_mode="networkidle"):
    with sync_playwright() as p:
        browser = p.chromium.launch()
        page = browser.new_page()
        page.goto(PAGE_URL, wait_until=wait_mode)
        if settle_ms > 0:
...
def pick_digit_boxes(mask, want_n):
    ...
    for c in cnts:
        ...
        if w < 14 or h < 14 or area < 120:  # noise
            continue
...
def run_once(outdir, tag):
    # load page
    page_path = os.path.join(outdir, f"page_{tag}.png")
    # try two modes: networkidle + domcontentloaded settle
    # here we already passed settle before calling this; this function just parses
    ...
    for label in PICKS:
        ...
        cv2.imwrite(os.path.join(outdir, f"{tag}_{label}_balls.png"), dbg)
        cv2.imwrite(os.path.join(outdir, f"{tag}_{label}_ymask.png"), ymask)
        ...
        for i,(x,y,w,h) in enumerate(boxes):
            ...
            cv2.imwrite(os.path.join(outdir, f"{tag}_{label}_digit{i+1}.png"), bin_img)
...
def main():
    ...
    # Shot A: immediate after networkidle
    shotA = os.path.join(run_id, "page_A.png")
    screenshot_page(shotA, settle_ms=0, wait_mode="networkidle")

    # Shot B: wait extra 3000 ms (3 sec) after load
    shotB = os.path.join(run_id, "page_B.png")
    screenshot_page(shotB, settle_ms=3000, wait_mode="networkidle")
